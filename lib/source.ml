...
let trim_row_loc (source : t) (row : int) : int = row |> max 0 |> min (List.length source.source - 1)
...
let insert (str : string) (source : t) : t =
  ...
  let rear_rows = Core.List.drop source.source (row + 1) in
  { source = front_rows @ [current_row] @ rear_rows; cursor = { row; col = col + List.length str } }

let split ?(with_display : bool = false) (source : t) : string * string =
  let lines = source.source in
  let lines = if with_display then [[]; []; []; []; []] @ lines @ [[]; []; []; []; []] else lines in
